...
echo '<div class="skidki"><p>Скидки при заказе!<ul><li>от 3 000 руб - 3%<li>от 5 000 руб - 5%<li>от 10 000 руб - 10%<li>от 15 000 руб - 15%<li>от 20 000 руб - 20%</ul></p></div>';
}
*/


add_action ('woocommerce_after_add_to_cart_form', 'paycards_text');

function paycards_text() {
echo '<div class="visa_txt"><p>Вы можете оплатить заказ на нашем сайте сразу после оформления с помощью банковской карты</p><img src="/wp-content/uploads/2019/02/logoPS_210.png"></div>';
}
...
echo '<p class="order-again">
	<a href="http://shkola-lepki.ru" class="button">Продолжить покупки</a>
</p>';
}
...
function add_coupon_message () {
 if (empty( WC()->cart->get_applied_coupons() )) {
  ...
  if ( $subt >= 3000 && $subt < 5000 ) {
     echo '<div class="cart-info-skidka">Уважаемый покупатель, Вам доступна скидка <b>3 %</b> за заказ на сумму свыше 3000 руб!<br>
Для активации скидки введите код <b>333</b> в поле "Код купона" выше и нажмите кнопку "Применить купон".<br>Стоимость заказа будет автоматически пересчитана с учетом скидки.<br><br>Применить купон к данному заказу можно только один раз.</div>';
     ...
     echo '<div class="cart-info-skidka">Уважаемый покупатель, Вам доступна скидка <b>5 %</b> за заказ на сумму свыше 5000 руб!<br>
...
} elseif ( $subt >= 10000 && $subt < 15000 ) {
     echo '<div class="cart-info-skidka">Уважаемый покупатель, Вам доступна скидка <b>10 %</b> за заказ на сумму свыше 10000 руб!<br>
...
} elseif ( $subt >= 15000 && $subt < 20000) {
    echo '<div class="cart-info-skidka">Уважаемый покупатель, Вам доступна скидка <b>15 %</b> за заказ на сумму свыше 15000 руб!<br>
Для активации скидки введите код <b>1515</b> в поле "Код купона" выше и нажмите кнопку "Применить купон".<br>Стоимость заказа будет автоматически пересчитана с учетом скидки.<br><br>Применить купон к данному заказу можно только один раз.</div>';
} elseif ( $subt >= 20000 && $subt < 100000) {
    echo '<div class="cart-info-skidka">Уважаемый покупатель, Вам доступна скидка <b>20 %</b> за заказ на сумму свыше 20000 руб!<br>
...
//add coupons description in admin emails
...
//add another email recipient for admin New Order emails
...
function add_another_recipient( $recipient, $order ) {
	...
	if ( $ship_method === 'Самовывоз со склада' ) {
		$recipient = $recipient . ',Sklad9425555@mail.ru';
	}
	return $recipient;
}
